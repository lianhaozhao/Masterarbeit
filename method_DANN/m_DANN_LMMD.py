...
def train_dann_lmmd(model,
                    ...
                    device, num_epochs=20, num_classes=10,batch_size=64,
                    pseudo_thresh=0.95,
                    scheduler=None,
                    grl_lambda_max = 1,
                    # Gating
                    lmmd_start_epoch=5,
                    lmmd_t=1,lmmd_lambda_max=0.3
                    ):
    ...
    for epoch in range(num_epochs):
        ...
        lambda_mmd_eff = mmd_lambda(epoch, num_epochs, max_lambda=lmmd_lambda_max, start_epoch=lmmd_start_epoch)
        ...
        dom_total_src = dom_total_tgt = mmd_steps = 0
        ...
        print(f"[Epoch {epoch+1}] Total loss:{avg_tot:.4f} | Avg cls loss:{avg_cls:.4f} | avg Dom loss:{avg_dom:.4f} "
              f"| avg LMMD loss:{avg_mmd:.4f} | DomAcc:{dom_acc:.4f} | "
...
if __name__ == "__main__":
    ...
    bs = 64
    lr = 0.0018140794566383449
    wd = 0.0015891351030900246
    ...
    for file in files:
        ...
        for run_id in range(10):
            ...
            ps_loader = get_pseudo_dataloaders(target_path=tgt_path, batch_size=bs)
            ...
            model = train_dann_lmmd(
                ...
                grl_lambda_max=1,
                lmmd_start_epoch=5,
                lmmd_t=0.5,lmmd_lambda_max=0.4338
            )
