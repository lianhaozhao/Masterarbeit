...
def suggest_adv_only(trial):
    params = {
        ...
        "grl_lambda_max":    trial.suggest_categorical("grl_lambda_max", [0.4,0.5,0.6,0.7,0.8]),
        "max_lambda":   trial.suggest_float("max_lambda", 0.35, 0.8, log=True),
        "pseudo_thresh": trial.suggest_categorical("pseudo_thresh", [0.90, 0.95]),
        "t": trial.suggest_categorical("t", [1.5, 2]),
...
def objective_adv_only(trial,
    ...
    best_val= train_dann_infomax_coral(
                model, src_tr, tgt_tr,src_va,
                optimizer, c_cls, c_dom, device,
                num_epochs=num_epochs, num_classes=10,
                scheduler=scheduler, batch_size=p["batch_size"],
                threshold=p["pseudo_thresh"],T=p["t"],coral_start_epoch=4,
                grl_lambda_max=p["grl_lambda_max"],max_lambda=p["max_lambda"],
            )
