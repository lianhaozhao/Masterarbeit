...
def dis_lambda(epoch, num_epochs, max_lambda=0.8, start_epoch=2):
    if epoch < start_epoch:
        return 0.0
    p = (epoch - start_epoch) / max(1, num_epochs - start_epoch)
    return (2 / (1 + np.exp(-6 * p)) - 1) * max_lambda
def div_lambda(epoch, num_epochs, max_lambda=1.0, start_epoch=1):
    p = (epoch - start_epoch) / max(1, num_epochs - start_epoch)
    return (2 / (1 + np.exp(-4 * p)) - 1) * max_lambda
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for it in range(iters):
            ...
            for _ in range(nC):
                ...
                loss_im = 0.8 *(loss_im_1 + loss_im_2)
                ...
                lambda_dis = div_lambda(epoch,num_epochs,max_lambda=1)
        ...
        if epoch > 10 and avg_lossC < best_score:
            best_score = avg_lossC
    ...
    for file in files:
        ...
        for run_id in range(10):
            ...
            model = train_mcd(
                ...
                save_dir=f"/content/drive/MyDrive/Masterarbeit/MCD_INFOMAX_LMMD/HC_T{file}",
                tag=f"RUN{run_id}"
            )
